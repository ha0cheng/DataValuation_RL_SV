...
class Dv_data(data.Dataset):
    def __init__(self, train_data_class, index_list, transform, p):
        self.transform = transform
        self.train_data = train_data_class.train_data[index_list[:int((1-p)*len(index_list))]]
        self.train_labels = np.array(train_data_class.train_labels)[index_list[:int((1-p)*len(index_list))]]
...
class Adult_dv(data.Dataset):
    def __init__(self, x_data, y_data, index_list, p):
        self.x_data = np.array(x_data)[index_list[int((1-p)*len(index_list)):]]
        self.y_data = np.array(y_data)[index_list[int((1-p)*len(index_list)):]]
